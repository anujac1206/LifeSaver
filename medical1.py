...
Advanced hospital administration and crisis management system
...
def run_medical(frame: np.ndarray, hand_pos: Optional[Tuple[int, int]], 
    ...
    img_y = col_y + 160
    img_h = 200
    cv2.rectangle(overlay, (col2_x + 20, img_y), (col2_x + col2_w - 20, img_y + img_h),
                  COLORS['bg_primary'], -1)
    cv2.rectangle(overlay, (col2_x + 20, img_y), (col2_x + col2_w - 20, img_y + img_h),
                  COLORS['accent_blue'], 2)
    
    # Image label
    img_label = phase['image_key'].replace('_', ' ').upper()
    cv2.putText(overlay, img_label, (col2_x + 30, img_y + 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, COLORS['text_dim'], 1, cv2.LINE_AA)
...
if __name__ == "__main__":
    ...
    print("Initialized successfully. Ready for integration with hand tracking.")
